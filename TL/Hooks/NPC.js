...
export class NPCHooks {
    ...
    static Initialize(info) {
       ...
       NPC.Spawner.TrySpawnAnNPC.hook((orig, self, player) => {
        
                    NPCLoader.EditSpawnRate(player, NPC.spawnRate, NPC.maxSpawnRate)
        return orig(self, player)
            ...
            // NPC.defaultSpawnRate // 600// 9,52% of chance spawn a enemy in a seconds.
            // 6
            
            /**if (player.calmed)
              {
                spawnRate = (int) ((double) spawnRate * 1.6499999761581421);
                maxSpawns = (int) ((double) maxSpawns * 0.60000002384185791);
              }
            */
        ...
        if (this.HookList.AI(info)) {
            Terraria.NPC['void AI()'
                ...
                NPCLoader._CachedForDraw = self
                ModNPC._CachedForAI = self
