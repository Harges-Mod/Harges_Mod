...
const NewProjectile = Projectile[
    'int NewProjectile(IEntitySource spawnSource, Vector2.new2 position, Vector2.new2 velocity, int Type, int Damage, float KnockBack, int Owner, float ai0, float ai1, float ai2, NewProjectileModifier modifer)'
];
...
const NewCombatText = CombatText[
...
export class HargesMMode extends ModPlayer {
    constructor() {
        ...
        this.MageCard = false;
        this.TheHighPriestessCard = false;
        this.EmpressCard = false;
        this.EmperorCard = false;
        this.HierophantCard = false;
        this.LoversCard = false;
        this.ChariotCard = false;
        this.StrengthCard = false;
        this.HermitCard = false;
        this.WheelOfFortuneCard = false;
        this.JusticeCard = false;
        this.HangedManCard = false;
        this.DeathCard = false;
        this.TemperanceCard = false;
        this.DevilCard = false;
        this.TowerCard = false;
        this.StarCard = false;
        this.MoonCard = false;
        this.SunCard = false;
        this.JudgmentCard = false;
        this.WorldCard = false;
    ...
    ModCombatText(Name, Color) {
        ...
        NewCombatText(player.getRect(), Color, `${Name}`, true, false);
    }
    ...
    UpdateEquips(player) {
        ...
        if (this.RubyFlame && !this.SupremeRuby) {
            if (Rand.NextBool(3)) {
                ...
                let particle =
                    GetRandomParticle[
                        (Math.random() * GetRandomParticle.length) | 0
                    ];

                Harges.Graphics.UParticle.Spawn(
                    Harges.Assets.Loader.Load(
                        `Assets/Adittive/${particle}.png`
                    ),

                    player.Center,

                    Vector2.new(
                        Math.random() * 0.2 * Rand.NextSign(),
            ...
            if (this.SupremeRubyCD >= Generic.toSec(5)) {
                if (
                    this.IsTargetValid(
                        this.currentTarget,
                        player.Center,
                        this.SupremeRubyRadius
                    )
                ) {
                    let direction = Vector2.Subtract(
        ...
        if (this.BloodyCover) {
            ...
            if (this.BloodyCoverAbstinenceTimer++ >= Generic.toSec(5)) {
                player.AddBuff(
                    ModBuff.getTypeByName("BloodAbstinence"),
                    60,
                    false
                );
            }
    ...
    OnFirstActivation() {
        ...
        const DarkSmileSound = NativeSound.New(
            "Assets/Sound/DarkSmile.mp3",
            "DarkSmile"
        );
                ...
                if (
                    ...
                    if (modPlayer) {
                        ...
                        if (
                            ...
                            modPlayer.SlimeSlink
                        ) {

                            proj.scale *=
                                modPlayer.SlimeSlinkBoostScaled;
                        }


                        if (
                            scaledProjectiles.size >
                            2000
                        ) {
