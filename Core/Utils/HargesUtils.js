...
export class HargesUtils  {
    ...
    dropItem = (condition, entity, Type, stack) => {
    ...
    toScreenPosition = vector2 => Vector2.Subtract(vector2, Generic.screenPosition())
    
    getOrigin(texture) {
        return Vector2.new(texture.Width / 2, texture.Height / 2);
    }
    
    NewParticle(position, velocity, color, scale, timeToLive = 60, rotation = 0) {
    ...
    getRect = (tex) => Rectangle.new(0, 0, tex.Width, tex.Height);
    
    EntityDraw(texture, position, sourceRectangle, color, rotation, origin, scale, effects) {
    ...
    NewProjectile(source, pos, vel, type, damage, knockback, owner = Main.myPlayer, ai0 = 0, ai1 = 0, ai2 = 0, modifier = null) {
